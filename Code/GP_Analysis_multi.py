...
def fit_minuit_gp(num,lnprob):
    ...
    for i in range(num):
        ...
        m = Minuit(lnprob,throw_nan=False,pedantic=False,print_level=0,Amp=init0,length=init1,
                    ...
                    limit_Amp = (100,1e15), limit_length = (4000,2000000))
...
h_chi2 = r.TH1D("h_chi2_1","Chi2 ad-hoc",100,0,20)
...
def BkgToyFit(l, return_dict):
    ...
    for t in range(Ntoys):
        ...
        Bkg_fit_function.SetParameters(1,1,-0.01,1e-6)
        Bkg_fit_function.FixParameter(0,1)
        fitresult = h_bkg.Fit(Bkg_fit_function,"SPRQ0")
        ...
        EffNdf = covmarprod.trace()
        
        chi2 = np.sum((toy-y_pred)**2/y_pred)
        chi2_fit[t] = chi2/(len(toy) - EffNdf)
    return_dict[l] = (l,np.mean(chi2_fit),np.std(chi2_fit),h_chi2.GetMean(),h_chi2.GetStdDev())
...
if __name__ == '__main__':
    ...
    proc = []
    for l in lum:
        p = multiprocessing.Process(target=BkgToyFit, args=(l,return_dict))
        proc.append(p)
        p.start()
    ...
    for i in range(len(return_dict.values())):
        if return_dict.values()[i][0] == 1:
            print(i)
            chi2_mean_gp[0] = return_dict.values()[i][1]
            chi2_mean_gp_err[0] = return_dict.values()[i][2]
            chi2_mean_par[0] = return_dict.values()[i][3]
            chi2_mean_par_err[0] = return_dict.values()[i][4]
        elif return_dict.values()[i][0] == 10:
            print(i)
            chi2_mean_gp[1] = return_dict.values()[i][1]
            chi2_mean_gp_err[1] = return_dict.values()[i][2]
            chi2_mean_par[1] = return_dict.values()[i][3]
            chi2_mean_par_err[1] = return_dict.values()[i][4]
        elif return_dict.values()[i][0] == 100:
            print(i)
            chi2_mean_gp[2] = return_dict.values()[i][1]
            chi2_mean_gp_err[2] = return_dict.values()[i][2]
            chi2_mean_par[2] = return_dict.values()[i][3]
            chi2_mean_par_err[2] = return_dict.values()[i][4]
    ...
    plt.errorbar(lum,chi2_mean_gp,yerr=chi2_mean_gp_err,color='maroon',label='GP',marker='.')
    ...
    plt.legend()
    plt.savefig(save_path+'test_statistic_evolution.png')
    plt.close()
    plt.figure(3)
    plt.plot(lum,np.linspace(1,1,len(lum)),'k')
    plt.errorbar(lum,chi2_mean_gp,yerr=chi2_mean_gp_err,color='maroon',label='GP',marker='.')
    plt.xlabel('Luminosity scale factor, GP only')
    plt.ylabel(r'$\chi^2/ndf$')
    plt.title(r'Test statistic evolution, GP only')
    plt.legend()
    plt.savefig(save_path+'test_statistic_evolution_GP.png')
    plt.close()
